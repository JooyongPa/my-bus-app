                  ...
                  {group.map((item) => (
                    <div key={item.arsId}
                      className={`flex items-center justify-between px-3 py-2 rounded mb-1 border ${
                        selectedStopId === item.arsId ? "bg-blue-900 border-blue-500" : "bg-zinc-800 border-transparent"
                      }`}>
                      <div>
                        {item.direction ? (
                          <div className="text-sm text-yellow-300 font-medium mb-0.5">→ {item.direction} 방면</div>
                        ) : null}
                        <div style={{ fontSize: "12px", color: "#aaa" }}>정류장번호: {item.arsId}</div>
                      </div>
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleSelectStop(item.arsId, item.stNm)}
                          className={`px-2 py-1 rounded text-sm text-white ${
                            selectedStopId === item.arsId ? "bg-gray-500" : "bg-blue-600 hover:bg-blue-700"
                          }`}>
                        ...
                        <button
                          ...
                          className={`px-2 py-1 rounded text-sm text-white ${
                            favorites.some((f) => f.id === item.arsId) ? "bg-gray-600 cursor-default" : "bg-green-600 hover:bg-green-700"
                          }`}>
